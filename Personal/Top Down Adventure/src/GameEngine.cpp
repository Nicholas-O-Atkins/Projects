...
{
	m_playerTreasures.push_back("Free Slot");
    init(path);
}
...
void GameEngine::addTreasures(const std::string newTreasure)
{
	m_playerTreasures.push_back(newTreasure);
}

const std::vector<std::string> & GameEngine::getTreasures() const
{
	return m_playerTreasures;
}